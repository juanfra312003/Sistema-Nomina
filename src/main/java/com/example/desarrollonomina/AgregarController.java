...
public class AgregarController {
    //Atributos implementados en la logica del programa y del modelo de negocio:
    private Stage stage;
    ...
    void registrarUsuario(ActionEvent event) throws IOException {
        ...
        if (posible(ingresarNombre.getText(), ingresarNombreAsignatura.getText(), ingresarCantidadHoras.getText())) {
            ...
            if (numHorasisDigit(ingresarCantidadHoras.getText())) {
                ...
                Integer numeroHoras = Integer.parseInt(ingresarCantidadHoras.getText());
                String nombreAsignatura = ingresarNombreAsignatura.getText();

                //Si es posible añadir la asignatura, mostrará el mensaje.
                if (nominaEmpleados.anadirAsignaturaProfesor(nombre, nombreAsignatura, numeroHoras)) {
                    Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
                    alert.setHeaderText("Profesor Registrado");
                    alert.setTitle("Registro Exitoso");
                    alert.setContentText("Se ha registrado la asignatura: " + nombreAsignatura + " al docente " + nombre + " de manera exitosa.");
                    alert.show();
                } else {
                    //En caso de que no sea posible crear el empleado se procedera a mostrar el mensaje de error.
                    Alert alert = new Alert(Alert.AlertType.ERROR);
                    alert.setHeaderText("Profesor Inexistente");
                    alert.setTitle("Error en la operacion");
                    alert.setContentText("El nombre ingresado no corresponde al registrado por parte de ningun empleado registrado como profesor.");
                    alert.show();
                }
            ...
            alert.setContentText("No has ingresado todas las credenciales para realizar el registro de una asignatura para un Profesor.");
            alert.show();
        }
    }
